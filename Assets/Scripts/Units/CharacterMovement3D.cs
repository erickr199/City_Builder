...
public class CharacterMovement3D : MonoBehaviour, ICharacterMovement
{
    [Header("Movement")]
    [SerializeField] private float _speed = 5f;
    [SerializeField] private float _acceleration = 10f;
    [SerializeField] private float _turnSpeed = 10f;
    [SerializeField] private bool _controlRotation = true;    // character turns towards movement direction

    [Header("Airborne")]
    [SerializeField] private float _gravity = -20f;       // custom gravity value
    [SerializeField] private float _jumpHeight = 2.25f;   // peak height of jump
    [SerializeField] private float _airControl = 0.1f;    // percentage of acceleration applied while airborne
    [SerializeField] private bool _airTurning = true;     // character can turn while airborne

    [Header("Grounding")]
    [SerializeField] private float _groundCheckOffset = 0.1f;     // height inside character where grounding ray starts
    [SerializeField] private float _groundCheckDistance = 0.4f;   // distance down from offset position
    [SerializeField] private float _maxSlopeAngle = 40f;          // maximum climbable slope, character will slip on anything higher
    [SerializeField] private float _groundedFudgeTime = 0.25f;    // leeway time for players to still jump after leaving the ground
    [SerializeField] private LayerMask _groundMask = 1 << 0;      // mask for layers considered the ground
